  ...
  return (
    <section className="p-5 md:p-8">
      <div className="max-w-3xl">
        ...
        <p className="mt-1 text-sm text-slate-500">Upload a work order PDF or start manually for a quotation/new customer.</p>
        ...
        <form onSubmit={onSubmit} className="mt-6 rounded-md border border-dashed border-slate-300 bg-white p-8 shadow-sm">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <input name="file" type="file" accept="application/pdf,.pdf" required className="w-full max-w-md" />
            {error ? <p className="text-sm font-medium text-red-600">{error}</p> : null}
            <button disabled={busy} className="btn-primary" type="submit">{busy ? "Parsing PDF..." : "Parse work order"}</button>
          </div>
        </form>
      </div>
    </section>
